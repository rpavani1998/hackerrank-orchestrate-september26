...
| Applying evidence to events and recurring streams | `code/evidence_extraction.py`: `apply_evidence_fact`; `code/main.py`: `load_evidence_facts`, `salary_evidence_applications`, `expand_resumed_salary`, `apply_relative_expense_amendments` | Validated facts, source metadata, event/series context | Status-only application, salary timeline projections, relative rent adjustments | Ledger, visibility, Decimal percent math, recurrence scope | Apply supported amendments to the correct event/series without double counting; keep unresolved facts inert | Resumed salary, sparse confirmed salary, 21-day cadence, and 12% rent-vs-arrears cases are tested; generic event mutation is not implemented |
...
| Recurrence detection and projection | `code/main.py`: `Agent.cadence`, `recurrence_series_key`, `recurring_projection`, `sparse_confirmed_salary_continuation`, `dedupe_salary_occurrences` | Historical settled events, category, description, dates, amounts | Future `ProjectionEvent` rows with source IDs and replacement reasons | Cadence thresholds, named-stream/category grouping, overlapping salary sources | Distinguish recurring commitments from one-time events and count each payday once | Sparse+recurring integration, second-employer same-day, and 21-day cadence cases verified |
